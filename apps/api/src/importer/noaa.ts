...
const buildStationRows = (
  ...
  for (const [stationId, station] of stations) {
    ...
    if (!inv?.tminFirst || !inv?.tminLast || !inv?.tmaxFirst || !inv?.tmaxLast) continue;
...
const flushAggregates = async (yearlyRows: Prisma.YearlyAggregateCreateManyInput[], seasonalRows: Prisma.SeasonalAggregateCreateManyInput[]) => {
...
const importDlyTar = async (tarGzPath: string) => {
...
const runImport = async () => {
  ...
  await prisma.$queryRaw`SELECT pg_advisory_lock(${BigInt(LOCK_KEY)})`;
  ...
  try {
    ...
    await prisma.seedMeta.upsert({
      ...
      create: { key: IMPORT_KEY, status: SeedImportStatus.RUNNING, endYear: END_YEAR, startedAt: new Date(), completedAt: null, error: null },
    ...
    await importDlyTar(tarPath);

    await prisma.seedMeta.update({ where: { key: IMPORT_KEY }, data: { status: SeedImportStatus.COMPLETED, completedAt: new Date(), error: null } });
    console.log('[importer] import completed');
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    await prisma.seedMeta.upsert({
      where: { key: IMPORT_KEY },
      create: { key: IMPORT_KEY, status: SeedImportStatus.FAILED, endYear: END_YEAR, startedAt: new Date(), completedAt: null, error: message },
      update: { status: SeedImportStatus.FAILED, completedAt: null, error: message },
    });
    throw error;
  } finally {
    await prisma.$queryRaw`SELECT pg_advisory_unlock(${BigInt(LOCK_KEY)})`;
